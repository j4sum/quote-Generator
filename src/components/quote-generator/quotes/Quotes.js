...
const Quotes = () => {
    ...
    return ( <
            div className = "container mt-2 pl-9" > { /* fetch random authors Data */ } <
            Link to = { `/${randAuthor}` }
            className = { `randAuthor${isActive ? :''}` }
            ...
            {!loading ? ( <
                    ...
                    quotes.map((quote, index) => ( <
                        p className = "font-medium text-lg md:text-xl mt-9 relative quote pl-8"
                        key = { index } > { quote.quoteText } <
                        /p>
                    ))
                } <
                /div>
